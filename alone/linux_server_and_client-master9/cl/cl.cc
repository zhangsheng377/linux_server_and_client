...
void MakeNewClient(struct sockaddr_in serverAddr,int epfd);
...
void MakeNewClient(struct sockaddr_in serverAddr,int epfd)
{
    ...
    scanf("%d",&ID);
    printf("Please input the user's life_time:");
    scanf("%lf",&life_time);
    printf("\n");
    ...
    char order[ORDER_LEN+1]="00";
    // 将信息发送给服务端
    strcat(send_message,"00");
    memcpy(&send_message[strlen(order)],&sendUser,sizeof(CLIENT));
    strcat(send_message,"");
